...
function Login({ onLogin }) {
  ...
  return <main className="admin-login"><section className="login-panel"><div className="brand"><img src={fleetosAdminLogo} alt="FleetOS" /></div><p className="secure-label"><span className="material-symbols-outlined">shield_lock</span> Restricted operations console</p><h1>Super Admin</h1><p className="muted">Company verification, platform access, and operational oversight.</p><form onSubmit={submit}><label>Email<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="admin@example.com" autoComplete="username" required /></label><label>Password<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" required autoFocus /></label>{error && <p className="error">{error}</p>}<button disabled={busy}>{busy ? 'Verifying…' : 'Enter secure console'}</button></form><small>Credentials are verified against the protected Super Admin record in MongoDB. Public users cannot create admin accounts.</small></section><aside><span className="material-symbols-outlined">admin_panel_settings</span><h2>One console.<br />Company verification.</h2><p>Review submitted documents, approve eligible companies, and manage portal access.</p></aside></main>;
}
...
function DocumentViewer({ viewer, onClose }) {
  ...
  return <div className="modal-backdrop document-backdrop" role="presentation"><section className="document-viewer" role="dialog" aria-modal="true" aria-label="Submitted business document"><header><div><p>Submitted business document</p><h2>{viewer.company.name}</h2><span>{document.name || 'Business license'}</span></div><button type="button" onClick={onClose} aria-label="Close document viewer"><span className="material-symbols-outlined">close</span></button></header><div className="document-canvas">{isImage ? <img src={document.data} alt={`${viewer.company.name} business document`} /> : <iframe title={`${viewer.company.name} business document`} src={document.data} />}</div><footer><span><span className="material-symbols-outlined">verified_user</span> Review the document, then close this viewer to approve or reject the company.</span><button type="button" onClick={onClose}>Close document</button></footer></section></div>;
}
...
function App() {
  ...
  useEffect(() => {
    const restoreSection = () => {
      const requested = window.location.hash.replace(/^#/, '');
      if (sectionIds.has(requested)) setSection(requested);
    };
  ...
  const requestMutation = (path, body, label) => { setAction({ path, body, label }); setError(''); };
  const confirmMutation = async (event) => {
    event.preventDefault();
    if (!action) return;
    setBusy(true); setError('');
    try { await api(action.path, { method: 'PATCH', body: JSON.stringify(action.body) }); setAction(null); await load(); }
    catch (requestError) { setError(requestError.message); }
    finally { setBusy(false); }
  };
  const openLicense = async (company) => {
    ...
    try {
      ...
      setDocumentViewer({ company, document: result.document });
    } catch (requestError) { setError(requestError.message); }
  };
  ...
  const render = () => {
    ...
    if (section === 'removal') return <section className="panel"><header><div><h2>Company access and client listing</h2><p className="muted">Block access suspends the owner account. Remove from client portal only hides the listing and preserves company records.</p></div><button onClick={() => download('company-access', current.companies || [])}>Export CSV</button></header><Table headers={['Company', 'Owner account', 'Client discovery', 'Portal access', 'Version', 'Actions']}>{(current.companies || []).map((company) => <tr key={company._id}><td><b>{company.name}</b><small>{company.city} · {company.email}</small></td><td>{company.owner?.name || '—'}<small>{company.owner?.status || 'unknown'}</small></td><td><Visibility company={company} /></td><td><Status value={company.owner?.status || 'suspended'} /></td><td>v{company.approvalVersion}</td><td className="action-cell">{company.owner?.status === 'active' ? <button className="danger" onClick={() => requestMutation(`/companies/${company._id}/status`, { status: 'suspended', version: company.approvalVersion }, `Block portal access for ${company.name}`)}>Block access</button> : <button onClick={() => requestMutation(`/companies/${company._id}/status`, { status: 'approved', version: company.approvalVersion }, `Restore portal access for ${company.name}`)}>Restore access</button>}{company.clientListed !== false ? <button onClick={() => requestMutation(`/companies/${company._id}/listing`, { clientListed: false }, `Remove ${company.name} from client portal`)}>Remove from client portal</button> : <button onClick={() => requestMutation(`/companies/${company._id}/listing`, { clientListed: true }, `Show ${company.name} in client portal`)}>Show in client portal</button>}</td></tr>)}</Table></section>;
