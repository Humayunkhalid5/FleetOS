...
const companyNav = (workspace = {}) => {
  const modules = { requests: true, workforce: true, inventory: true, customers: true, chat: true, reviews: true, analytics: true, ...workspace.modules };
  return [
  ['space_dashboard', 'Overview', ROUTES.companyDashboard],
  modules.requests && ['handyman', workspace.requestLabel || 'Requests', ROUTES.companyBookings],
  modules.workforce && ['badge', workspace.workforceLabel || 'Staff', ROUTES.companyTechnicians],
  modules.inventory && ['inventory_2', workspace.inventoryLabel || 'Inventory', ROUTES.companyInventory],
  ['storefront', workspace.offeringLabel || 'Services', ROUTES.companyServices],
  modules.customers && ['groups', 'Customers', ROUTES.companyCustomers],
  modules.chat && ['forum', 'Messages', ROUTES.companyChat, 'messages'],
  modules.reviews && ['star', 'Reviews', ROUTES.companyReviews],
  modules.analytics && ['monitoring', 'Analytics', ROUTES.companyAnalytics],
...
function CompanyShell({ title, subtitle, actions, search, onSearch, children }) {
  ...
  const navigation = companyNav(company?.workspace);

  return (
    <div className="min-h-screen bg-[#f4f7fb] text-slate-800 font-sans md:flex">
      <aside className="company-sidebar hidden md:flex fixed inset-y-0 left-0 w-[218px] bg-white border-r border-slate-100 flex-col z-50 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
        <button onClick={() => navigate(ROUTES.companyDetails)} className="company-brand h-[76px] px-5 flex items-center text-left hover:bg-slate-50 transition-colors">
          <img src={fleetosLogo} alt="FleetOS" />
        </button>

        <div className="company-sidebar-scroll px-4 py-3 overflow-y-auto flex-1">
          ...
          <nav className="space-y-1">
            {navigation.map(([icon, label, to, badge]) => {
      ...
      <main className="flex-1 md:ml-[218px] min-w-0 overflow-x-hidden">
        <header className="min-h-[72px] px-4 md:px-6 py-3 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between sticky top-0 z-40 bg-[#f4f7fb]/85 backdrop-blur-xl border-b border-white/60">
          <div>
