...
function CompanyChat() {
  ...
  const loadMessages = useCallback(async () => {
    ...
    try { const result = await api.get(`/chats/${active._id}/messages`, { noCache: true }); setMessages(result.messages || []); }
    catch (requestError) { setError(requestError.message); }
  }, [active?._id]);
    ...
    <CompanyShell title="Client Messages" subtitle="Reply to booking conversations and keep customers updated in real time.">
      ...
      <div className="h-[calc(100vh-150px)] min-h-[620px] bg-white border border-slate-100 rounded-3xl overflow-hidden grid lg:grid-cols-[360px_1fr] shadow-[0_4px_24px_rgba(0,0,0,0.02)]">
        <aside className="border-r border-slate-100 overflow-y-auto bg-white">
          ...
          ) : conversations.map(({ booking, lastMessage }) => (
        ...
        <section className="flex flex-col min-w-0 bg-slate-50/70">
          {active ? (
            <>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-3">
                {messages.map((message) => (
                  <div key={message._id} className={`max-w-[78%] p-3.5 rounded-2xl text-sm shadow-sm ${message.senderRole === 'company' ? 'ml-auto bg-blue-600 text-white rounded-br-md' : 'bg-white border border-slate-100 text-slate-800 rounded-bl-md'}`}>
                    <p className="leading-6">{message.text}</p>
                    <small className="block mt-1 opacity-60 text-[10px] font-semibold">{new Date(message.createdAt).toLocaleTimeString()}</small>
                  </div>
                ))}
              </div>
              <form onSubmit={send} className="p-4 border-t border-slate-100 bg-white flex gap-3">
